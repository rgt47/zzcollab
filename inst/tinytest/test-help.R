...
# zzcollab_help rejects invalid topics
...
# zzcollab_help works with NULL (general help)
result <- tryCatch({
  zzcollab_help(NULL)
}, error = function(e) {
...
# zzcollab_help lists current valid topics in error
result <- tryCatch({
...
# zzcollab_help routes topics via the 'help' subcommand
captured <- NULL
local_mocked_bindings(
  find_zzcollab_script = function() "zzcollab",
  safe_system = function(command, ...) {
