...
RENV_EOF
    ...
    log_info "setup_renv.R already exists, skipping creation"
fi
...
if [ -f "renv.lock" ]; then
    ...
    echo "No renv.lock found, using latest R version from rocker/verse"
...
PACKAGE_WORKFLOW_EOF
    else
        log_info "r-package.yml already exists, skipping creation"
    fi
...
WORKFLOW_EOF
        ...
        log_info "render-paper.yml already exists, skipping creation"
    fi
}
...
local features=(
    ...
    "- Research paper template in analysis/paper/ with PDF output"
    "- Comprehensive user guide in RRTOOLS_USER_GUIDE.md"
...
local dev_workflow=(
    "• Development: ''make docker-r'' → do R work → exit container"
