...
- The output of `zzcollab --version` and `bash --version` (CLI side) or
  [`sessionInfo()`](https://rdrr.io/r/utils/sessionInfo.html) (R side).
...
approach. zzcollab has well-defined extension points (Docker profiles in
`bundles.yaml`, configuration layers, the auto-snapshot pipeline); a
brief design discussion saves rework.
...
- Modules in `modules/` follow the existing `core.sh` / `cli.sh`
  pattern: each function is documented at the top with purpose, inputs,
  and outputs.
...
Conduct](https://rgt47.github.io/zzcollab/CODE_OF_CONDUCT.md).
