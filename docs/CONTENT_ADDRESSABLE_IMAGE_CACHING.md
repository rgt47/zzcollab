...
### Why Remove Metadata Comments from Dockerfile?

Earlier versions of zzcollab added a comment to generated Dockerfiles:

```dockerfile
# Generated by zzcollab - Profile: analysis - Packages: dplyr ggplot2
```

This comment included project-specific information (package names), causing
Dockerfiles with identical functional content to produce different hashes.
Two projects using the same profile and R version would fail to match because
their package lists differed in the comment.

**Solution**: Remove all metadata comments from the Dockerfile. The Dockerfile
now contains only functional instructions. Profile detection for status
messages is derived from the base image name instead:

```bash
# Derive profile from base image (e.g., rocker/tidyverse -> analysis)
PROFILE=$(echo "$BASE_IMAGE" | sed 's|.*/||; s|:.*||; s|tidyverse|analysis|')
```

This ensures that any two projects with identical Dockerfile + renv.lock
content will produce the same hash and share cached images.
