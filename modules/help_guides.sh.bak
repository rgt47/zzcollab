...
show_renv_help() {
    show_renv_help_content | smart_pager
}
...
show_build_modes_help() {
    show_build_modes_help_content | smart_pager
}
...
show_docker_help() {
    show_docker_help_content | smart_pager  
}
...
show_cicd_help() {
    show_cicd_help_content | smart_pager
}
...
if ! command -v smart_pager &>/dev/null && ! declare -f smart_pager >/dev/null 2>&1; then
    log_error "help_guides.sh: smart_pager function not available"
    log_error "help_guides.sh: Required module not loaded properly"
    return 1
fi
