...
# USAGE:    create_core_files [pkg_name] [profile_name] [templates_dir]
# ARGS:
#   $1 - pkg_name: Package name (optional, defaults to PKG_NAME global)
#   $2 - profile_name: Docker profile name (optional, defaults to PROFILE_NAME global)
#   $3 - templates_dir: Templates directory (optional, defaults to TEMPLATES_DIR global)
# RETURNS:
#   0 - All core files created successfully
#   1 - Creation failed (missing parameters, template not found, etc.)
# GLOBALS:
#   READ: PKG_NAME (if $1 not provided), PROFILE_NAME (if $2 not provided),
#         TEMPLATES_DIR (if $3 not provided)
#   WRITE: None
# Creates:
#   - DESCRIPTION file with package metadata
...
create_core_files() {
    ...
    local profile_name="${2:-$PROFILE_NAME}"
    local templates_dir="${3:-$TEMPLATES_DIR}"
    local year=$(date +%Y)

    # Validate required parameters
    if [[ -z "$pkg_name" ]]; then
        log_error "create_core_files: pkg_name parameter not provided and PKG_NAME global not set"
        return 1
    fi

    if [[ -z "$profile_name" ]]; then
        log_error "create_core_files: profile_name parameter not provided and PROFILE_NAME global not set"
        return 1
    fi

    if [[ -z "$templates_dir" ]]; then
        log_error "create_core_files: templates_dir parameter not provided and TEMPLATES_DIR global not set"
        return 1
    fi
...
# USAGE:    create_readme_file [pkg_name] [templates_dir]
# ARGS:
#   $1 - pkg_name: Package name (optional, defaults to PKG_NAME global)
#   $2 - templates_dir: Templates directory (optional, defaults to TEMPLATES_DIR global)
# RETURNS:
#   0 - README.md created successfully
#   1 - Creation failed
# GLOBALS:
#   READ: PKG_NAME (if $1 not provided), TEMPLATES_DIR (if $2 not provided)
#   WRITE: None
# Creates:
...
create_readme_file() {
    local pkg_name="${1:-$PKG_NAME}"
    local templates_dir="${2:-$TEMPLATES_DIR}"
...
# USAGE:    create_renv_setup [templates_dir]
# ARGS:
#   $1 - templates_dir: Templates directory (optional, defaults to TEMPLATES_DIR global)
# RETURNS:
#   0 - renv setup created successfully
#   1 - Creation failed
# GLOBALS:
#   READ: TEMPLATES_DIR (if $1 not provided)
#   WRITE: None
# Creates:
...
create_renv_setup() {
    local templates_dir="${1:-$TEMPLATES_DIR}"
