...
>   (e.g. `analysis`, the tidyverse profile).
> - **Base image**: the starting Docker image a project builds on; here
>   extended via `--base-image` to add X11 or vim tooling.
> - **`.Rprofile`**: the R session-configuration file, here used to load
>   the plotting helper functions automatically.
...
- [Kitty terminal](https://sw.kovidgoyal.net/kitty/) installed on your
  host
- Docker with a ZZCOLLAB profile (e.g., `analysis`). Terminal editors
  such as vim and X11 tooling can be added via `--base-image` or
  `.Rprofile.local`
...
docker run -it -p 8888:8888 -v $(pwd):/home/analyst/project myproject
```
...
### The Challenge
    ...
    │ Docker Container                                │
    │   R, ggplot2                                    │
    │   Project mounted at: /home/analyst/project     │
...
| Configuration                                    | Size   | Graphics Method |
|--------------------------------------------------|--------|-----------------|
| `analysis` profile                               | ~1.2GB | kitty/httpgd    |
| `analysis` profile plus X11 (via `--base-image`) | ~2.2GB | X11 forwarding  |
