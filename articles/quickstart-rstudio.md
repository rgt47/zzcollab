...
- Development tooling baked into the image (not renv.lock): the R
  `languageserver` for in-container completion and diagnostics (default
  on; opt out with `zzc config set languageserver false`)
- System libraries: git, pandoc, LaTeX (basic). For full PDF/manuscript
  rendering use the `publishing` profile (rocker/verse), which pre-bakes
  the LaTeX closure
