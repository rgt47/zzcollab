# Scholarly Manuscript Development with ZZCOLLAB

## Introduction

This vignette demonstrates manuscript development using the unified
research compendium framework (Marwick et al. 2018). The key insight is
that **the manuscript is the organizing principle**—you start writing
from day one, and the compendium directories (`R/`, `tests/`,
`analysis/scripts/`) emerge naturally as code matures.
...
> - **Container / Dockerfile**: a packaged software environment and the
>   recipe that builds it, so the manuscript renders identically
>   elsewhere.
> - **`renv.lock`**: a JSON record of every package version needed to
>   rebuild the environment.
> - **`R/`**: the directory holding reusable functions extracted from
>   inline manuscript code.
> - **Unit test / tinytest**: a focused check that one piece of code
>   behaves as intended; zzcollab projects use tinytest.
> - **zzrenvcheck**: zzcollab’s dependency check verifying every package
>   used in code is declared in `DESCRIPTION` and pinned in `renv.lock`.
...
5.  Add tests to `inst/tinytest/` as functions solidify
...
### Overview
    ...
    ├── inst/
    │   └── tinytest/             # Unit tests (add as functions solidify)
...
| `inst/tinytest/` | Unit tests | When functions are stable |
...
# R/data_functions.R has tests in inst/tinytest/test_data_functions.R
```
...
before and after the session, so package usage stays consistent with
...
When you find yourself copying code or using similar patterns, extract
to `R/`:
...
This project uses tinytest: test files live in `inst/tinytest/` and
contain bare top-level expectations (no `test_that()` wrapper).

**Create `inst/tinytest/test_scaling_functions.R`**:

``` r

# extract_allometric_slopes returns correct structure
...
# extract_allometric_slopes handles missing values
...
result_missing <- extract_allometric_slopes(missing_data, y, x, group)
expect_equal(nrow(result_missing), 2)

# test_isometry correctly identifies scaling types
expect_equal(test_isometry(slope = 1.0, se = 0.05)$scaling_type, 'isometric')
expect_equal(
  test_isometry(slope = 0.7, se = 0.05)$scaling_type,
  'negative allometry'
)
expect_equal(
  test_isometry(slope = 1.3, se = 0.05)$scaling_type,
  'positive allometry'
)

# test_isometry returns correct p-value for known case
# slope = 1.0, se = 0.1 -> t = 0 -> p = 1
iso <- test_isometry(slope = 1.0, se = 0.1)
expect_equal(iso$t_statistic, 0)
expect_equal(iso$p_value, 1)
```
...
After several weeks of development, your project might look like:
    ...
    ├── inst/
    │   └── tinytest/
    │       └── test_scaling_functions.R   # Unit tests
    ├── DESCRIPTION
    ├── Dockerfile
    ├── Makefile
    └── renv.lock
...
1.  **Start with the manuscript**: Create `report.Rmd` on day one. Write
    your research question, background, and planned methods before any
    analysis.

2.  **Code inline first**: Keep code in `report.Rmd` while exploring.
    Move to `R/` only when patterns emerge and code is reused.

3.  **Extract functions when ready**: When you copy-paste code or use
    similar logic in multiple places, create a function in `R/`.

4.  **Test critical code**: Add tests in `inst/tinytest/` for functions
    that directly produce results in the manuscript.

5.  **Cache expensive computations**: Move slow code to
    `analysis/scripts/` and save results to
    `analysis/data/derived_data/`.

6.  **The manuscript is the product**: Everything else supports it. The
    compendium structure emerges from the writing process, not the
    reverse.
