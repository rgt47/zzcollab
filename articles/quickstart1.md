...
>   into the container): minimal, tidyverse, rstudio, or publishing
>   (`analysis` is a deprecated alias for `tidyverse`). Distinct from
>   the archetype.
...
> - **Auto-snapshot**: the automatic
>   [`renv::snapshot()`](https://rstudio.github.io/renv/reference/snapshot.html)
>   that runs when you exit R in the container, capturing new packages
>   into `renv.lock`.
...
for the full toggle model). For this tutorial, accept the recommended
renv plus Docker.
...
- An **archetype** is the project type and layout (chosen in Question
  1).
- A **profile** is the Docker image bundle, that is, which R packages
...
**Available profiles** (the four canonical profiles):

- `minimal` - Command-line R environment (~650MB)
- `tidyverse` - Tidyverse data analysis (~1.2GB), recommended (formerly
  `analysis`; the `analysis` alias is still accepted)
- `rstudio` - RStudio Server (~980MB)
- `publishing` - Manuscript rendering with LaTeX pre-baked (~4.2GB);
  renders PDF when a LaTeX toolchain is present, HTML otherwise

For modeling, Shiny, or other domains, start with the closest one and
add packages inside the container with
...
# Grow the manifest from the image before committing: hydrate,
# renv::snapshot(), then zzrenvcheck (strict + auto-fix)
make snapshot
...
`renv.lock` and `DESCRIPTION` play complementary roles: `renv.lock` pins
the exact package closure (versions), while `DESCRIPTION` declares
dependency roles. Packages used by code in `R/` belong in `Imports`;
packages used only by `analysis/` (reports, scripts, notebooks) belong
in `Suggests`. `make snapshot` and `zzrenvcheck` keep both honest.
...
2.  Validate package dependencies in a step named ‘Validate dependency
    manifest (renv.lock + DESCRIPTION)’, which runs zzrenvcheck
3.  Run unit tests
4.  Report results

**Note**: the ‘Validate dependency manifest (renv.lock + DESCRIPTION)’
step invokes `zzrenvcheck::check_packages()` inside the container and
fails the build when code references a package that is not declared in
`DESCRIPTION` or not locked in `renv.lock`.
...
Makefile, CI workflows, the `.Rprofile` template, and similar). Two
minor maintenance commands keep those in step with the installed
zzcollab version:
...
# Development tooling lives in the image, never in renv.lock. The R
# language server (languageserver, on by default) provides in-container
# LSP; styler and lintr back make style / make lint.
RUN R -e "install.packages(c('languageserver', 'styler', 'lintr'))"

RUN mkdir -p /opt/renv/library /opt/renv/cache
COPY renv.lock renv.lock
# Reproducibility-relevant packages, including yaml and here, are restored
# from renv.lock rather than installed explicitly here.
RUN R -e "renv::init(bare=TRUE, force=TRUE, restart=FALSE); renv::restore()"
```

The split above follows a rule: development tooling (languageserver,
styler, lintr, devtools, roxygen2, …) belongs in the Dockerfile and
never in `renv.lock`, while reproducibility-relevant packages belong in
`renv.lock` and `DESCRIPTION`. See
`docs/package-placement-whitepaper.md` for the full account of which
packages go in the Dockerfile versus `renv.lock` versus `DESCRIPTION`.
...
A profile is the Docker image bundle (which R packages are baked into
the container). It is separate from the archetype (the project type) and
from the features you toggle.
...
hashing, code-quality hooks, unit tests, and cloud launch, are
*features* you turn on and off at any time. Use the interactive wizard,
or the explicit `add` / `rm` forms:
...
and the `zzc verify` workflow are documented in
[`vignette("quickstart3")`](https://rgt47.github.io/zzcollab/articles/quickstart3.md).
...
- Packages are added dynamically using
  [`install.packages()`](https://rdrr.io/r/utils/install.packages.html)
  inside containers.
- **Auto-snapshot**: packages are captured in `renv.lock` when you exit
  R in the container.
...
[`vignette("quickstart3")`](https://rgt47.github.io/zzcollab/articles/quickstart3.md).
For the capture/validation layers in depth, see
[`vignette("reproducibility-layers")`](https://rgt47.github.io/zzcollab/articles/reproducibility-layers.md).

------------------------------------------------------------------------

*Rendered on 2026-06-19 at 08:36 PDT.*  
