# Plot Viewing with zzvim-R Terminal Graphics

## Overview

The `zzvim-R` vim plugin provides automatic plot display in kitty and
iTerm2 terminals. Unlike manual save-and-view approaches, zzvim-R
...
- [zzvim-R plugin](https://github.com/rgt47/zzvim-R) installed in
  vim/neovim
...
When you open vim in a kitty terminal within an R project, zzvim-R
automatically:
...
Plots appear directly in the terminal without manual
`kitty +kitten icat` calls.

### How It Works

zzvim-R overrides [`dev.off()`](https://rdrr.io/r/grDevices/dev.html) to
automatically display plots after rendering:
...
When using zzcollab Docker containers with kitty, the terminal graphics
work automatically if `.Rprofile.local` is in the mounted project
directory.
...
Verify kitty remote control is enabled. Add to
...
For simple workflows, the manual `kp()` function (see
[`vignette("workflow-kitty-graphics")`](https://rgt47.github.io/zzcollab/articles/workflow-kitty-graphics.md))
may be sufficient. For vim-based development with frequent plotting,
zzvim-R provides a more integrated experience.
...
- [ZZCOLLAB Kitty Graphics
  Workflow](https://rgt47.github.io/zzcollab/articles/workflow-kitty-graphics.md)
