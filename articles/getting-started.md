...
**minimal**: Lightweight development environment (~650MB) - Base:
rocker/r-ver - Packages: renv, devtools, usethis, tinytest, roxygen2

**tidyverse**: Data analysis environment (~1.2GB, recommended) - Base:
rocker/tidyverse - Packages: renv, devtools, tidyverse, here - Formerly
named `analysis`; the `analysis` alias is still accepted

**rstudio**: RStudio Server in the browser (~980MB) - Base:
rocker/rstudio - Packages: renv, devtools, usethis, tinytest, roxygen2

**publishing**: Manuscript rendering (~4.2GB) - Base: rocker/verse -
Packages: renv, devtools, rmarkdown, bookdown, plus a pre-baked LaTeX
toolchain - Renders PDF when a LaTeX toolchain is present and falls back
...
To build on a different base image (for example, an organization image
with packages pre-installed), pass `--base-image` to the `docker`
command:
...
Implement the following test suite. ZZCOLLAB projects use tinytest, so
tests are bare top-level expectations with no `test_that()` wrapper:
...
ZZCOLLAB tracks dependencies across two complementary files. `renv.lock`
pins the exact package closure (every version needed to rebuild the
environment), while `DESCRIPTION` declares dependency roles: packages
...
files are complementary, not redundant, and both must stay honest for
reproducibility to hold.

ZZCOLLAB provides automated dependency checking to keep them aligned.
The check is implemented by the companion R package `zzrenvcheck`, which
is installed into the project image. `make check-renv` runs
...
To grow the manifest from the current image rather than only validate
it, run `make snapshot` before committing. Inside the container it runs
...
newly used packages into `renv.lock` and `DESCRIPTION` in one step:

``` bash
# Hydrate, snapshot, and check dependencies (run before commit)
make snapshot
```
...
automated checking on every commit. The R-package workflow includes a
step named “Validate dependency manifest (renv.lock + DESCRIPTION)” that
runs `zzrenvcheck::check_packages` and fails the build when code
references a package that is not declared in `DESCRIPTION` or not locked
in `renv.lock`. The workflow validates:
...
| tidyverse  | ~1.2GB | General analyses (recommended) | rocker/tidyverse |
| rstudio    | ~980MB | RStudio Server                 | rocker/rstudio   |
| publishing | ~4.2GB | Manuscript rendering (PDF)     | rocker/verse     |

The `tidyverse` profile was formerly named `analysis`; the `analysis`
