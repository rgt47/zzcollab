...
2.  [Testing Philosophy for Data
    Analysis](#testing-philosophy-for-data-analysis)
3.  [Testing Framework Overview](#testing-framework-overview)
4.  [Types of Tests for Data
    Analysis](#types-of-tests-for-data-analysis)
5.  [Setting Up Your Testing
    Environment](#setting-up-your-testing-environment)
6.  [Unit Testing for Analysis
    Functions](#unit-testing-for-analysis-functions)
7.  [Integration Testing for Data
    Pipelines](#integration-testing-for-data-pipelines)
8.  [Data Validation and Quality
    Testing](#data-validation-and-quality-testing)
...
12. [Continuous Integration for Data
    Analysis](#continuous-integration-for-data-analysis)
13. [Best Practices and Common
    Pitfalls](#best-practices-and-common-pitfalls)
...
1.  **Computational Reproducibility**: Ensuring another researcher can
    use your code and data to independently obtain identical results
...
Following the reproducible research workflow principles (Poldrack,
2019), testing should align with the three phases of data analysis:
...
These two files play complementary roles: `renv.lock` pins the exact
package closure, while `DESCRIPTION` declares dependency roles (packages
used by `R/` code go in `Imports`; packages used only by `analysis/`
reports and scripts go in `Suggests`). To grow both from the current
image rather than only validate them, run `make snapshot` before
committing; it runs
[`renv::hydrate()`](https://rstudio.github.io/renv/reference/hydrate.html),
`renv::snapshot(prompt = FALSE)`, and the same `zzrenvcheck` check in
sequence. In CI, the R-package workflow enforces the manifest with a
step named “Validate dependency manifest (renv.lock + DESCRIPTION)” that
fails the build when code references a package absent from `DESCRIPTION`
or `renv.lock`.

#### Code Quality: make style and make lint

When the code-quality feature is enabled, styler and lintr are baked
into the project image, and two targets run them inside the container:
...
Both targets run in the container, so no host R installation is
required. They are gated on the code-quality feature: if it is not
enabled, the tooling is absent from the image and the targets are
unavailable. Running `make style` before committing keeps formatting
consistent, and `make lint` surfaces style and correctness issues that
tests may not catch.
...
1.  **Test Early and Often**: Implement tests during the exploratory
...
2.  **Overly Specific Tests**: Avoid tests that break with minor,
    acceptable changes
3.  **Ignoring Random Elements**: Always set seeds when testing
    functions with randomness
4.  **Insufficient Data Validation**: Test data assumptions explicitly
5.  **Skipping Integration Tests**: Unit tests alone do not catch
    pipeline failures
...
- **validate**: Rule-based data validation (van der Loo & de Jonge,
  2021)
- **assertr**: Pipeline assertions and data verification (Fischetti,
  2021)
...
Guy, R. T., … & Wilson, P. (2014). Best practices for scientific
