# Simulation Studies for Longitudinal Binary Data

## Introduction

This vignette demonstrates a simulation study workflow using the Write
as You Go approach. The key insight is that **the manuscript organizes
the simulation from the beginning**—you start with your research
...
### The Compendium Structure for Simulations
    ...
    ├── inst/
    │   └── tinytest/                 # Tests for each R/ file
    ├── DESCRIPTION
    ├── Dockerfile
    └── renv.lock
...
abstract: |
  **Background**: Longitudinal studies with binary outcomes require methods
  that account for within-subject correlation.
  **Methods**: We conducted a simulation study comparing five methods.
  **Results**: [To be completed]
---
```

Then add your setup chunk and content. Document your simulation
parameters in a table:

``` r

# In dgm-parameters chunk:
params <- data.frame(
...
Writing the Methods section first forces you to think through the
simulation design before writing any code.

## Step 2: Implement Data Generation Inline

Add data generation code directly in the manuscript to develop and test
it:

``` r

# In simulate-inline chunk (echo=TRUE):
...
Use inline R to report results in the prose.
...
Use inline R to report bias values in the prose.
...
This project uses tinytest. Create `inst/tinytest/test_simulate_data.R`
with bare top-level expectations (no `test_that()` wrapper):

``` r

# simulate_longitudinal_binary returns correct structure
...
# simulate_longitudinal_binary respects seed
...
# simulate_longitudinal_binary produces valid outcomes
data_valid <- simulate_longitudinal_binary(n_subjects = 100, seed = 42)
expect_true(all(data_valid$y %in% c(0, 1)))

# create_simulation_grid produces correct dimensions
...
Once the simulation packages (`furrr`, `geepack`, `lme4`, `progressr`,
and the rest) are installed, lock them into the manifest before
committing:
...
performance |>
  ...
  labs(x = "Sample Size", y = "Power", color = "Method", linetype = "sigma") +
  theme_minimal()
```

## Summary: Write as You Go for Simulations

1.  **Start with the manuscript**: Write your research question and
    ADEMP framework before any code.

2.  **Develop inline**: Implement data generation and model fitting
    directly in `report.Rmd` first. Test immediately with small
    examples.
...
5.  **Cache full results**: Move production simulations to
    `analysis/scripts/` and save results to
    `analysis/data/derived_data/`.

6.  **Load and report**: The manuscript loads cached results and
    presents them. Changes to presentation don’t require re-running
    simulations.
...
- `inst/tinytest/` - Unit tests for each module
