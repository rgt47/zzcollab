...
# - renv.lock automatically updated and validated (pure shell)
...
ZZCOLLAB provides 14+ predefined Docker profiles to balance image size
and capabilities:
...
### Additional Structure
    ...
    ├── modules/
    │   └── validation.sh               # Pure shell dependency validation (NO R REQUIRED!)
    ├── tests/
    │   └── testthat/
...
# Validate before committing (pure shell - NO R REQUIRED!)
...
# 2. Validates package dependencies (pure shell validation)
# 3. Runs R CMD check
...
# Validates:
#   - Package dependencies synchronized (pure shell validation via make check-renv)
#   - R CMD check passes
#   - Test suite passes
...
# - Validates dependencies (pure shell validation)
# - Runs all tests
...
``` bash
# Check that all packages used in code are declared (pure shell - NO R REQUIRED!)
make check-renv  # Pure shell validation (grep, awk, jq)
...
``` bash
# Team lead setup with geospatial profile
...
jobs:
  check:
    ...
    steps:
      ...
      - name: Validate dependencies (pure shell - NO R REQUIRED!)
        run: make check-renv  # Pure shell validation (grep, awk, jq)
```
...
# Right workflow (pure shell validation - NO R REQUIRED!)
make check-renv  # Pure shell validation (grep, awk, jq)
make docker-test
git commit -m "Fix analysis"
git push  # CI passes
