...
# Framework-managed files that 'zzc update' regenerates from the current
# templates. Format: "template-relative-path|destination-relative-path". The
# Dockerfile is deliberately excluded: its content is derived from renv.lock,
# base image, and install mode (regenerated by 'zzc docker'), not a static
# template. User-owned paths (R/, analysis/, DESCRIPTION, renv.lock, data/)
...
Regenerate the framework-managed template files (Makefile, .Rprofile, CI
workflows, user guide) from the current templates, re-stamping them to the
current template version. The existing Posit Package Manager pin is preserved.

Never touches user content (R/, analysis/, DESCRIPTION, renv.lock, data/). The
Dockerfile is not regenerated here -- run 'zzcollab docker' to refresh it.
...
EOF
    ...
    if [[ -f "$dir/Dockerfile" ]]; then
        log_info "Dockerfile present but not regenerated by 'update' (its content derives"
        log_info "from renv.lock, base image, and install mode). Run 'zzcollab docker' to refresh it."
    fi
