...
install.packages("dplyr")
install.packages("ggplot2")
# Or use renv::install() for GitHub packages: renv::install("user/package")
...
- **Minimal learning curve**: Familiar R workflow with `install.packages()`
...
- **Snapshot discipline**: Must remember `renv::snapshot()` after package
  changes (automated in ZZCOLLAB Docker containers - see Level 3)
...
zzcollab -t mylab -p climate-study --profile-name geospatial
...
cd climate-study
zzcollab --use-team-image  # Gets team's Docker image
make r
...
zzcollab --profile-name analysis --create-github-repo
...
zzcollab -t mylab -p paper2024 --profile-name analysis --create-github-repo
...
zzcollab -t climate-team -p precipitation --profile-name geospatial
...
zzcollab -t myname -p mypackage --profile-name analysis --create-github-repo
...
**Forgetting `renv::snapshot()`** (Note: Automated in ZZCOLLAB Docker!):
...
**Package installation methods** (both work in ZZCOLLAB Docker containers):

```r
# Standard R command (recommended in ZZCOLLAB Docker containers)
install.packages("dplyr")  # Auto-captured in renv.lock on exit

# Alternative: renv command (works for CRAN and GitHub packages)
renv::install("dplyr")     # Also captured in renv.lock

# For GitHub packages
renv::install("user/package")
```

**Note**: In ZZCOLLAB Docker containers, `install.packages()` works correctly
because packages are automatically captured in renv.lock when you exit the
container. Outside of Docker, use `renv::install()` to ensure packages go
