---
title: "ZZCOLLAB Configuration System Guide"
...
header-includes:
  ...
  - \newunicodechar{✓}{$\checkmark$}
vignette: >
  %\VignetteIndexEntry{ZZCOLLAB Configuration System Guide}
...
- **✓ Eliminate repetitive typing**: Set defaults once, use across all projects
- **✓ Team standardization**: Shared configurations ensure identical environments
- **✓ Flexible customization**: 14+ Docker variants, custom package lists, build modes
- **✓ Hierarchical control**: User, project, and system-level configurations
- **✓ Single source of truth**: Centralized variant definitions prevent duplication
...
- **Interactive variant browser** for easy discovery and selection

### 📦 Package Management System
Control what R packages are installed and when:
- **Build mode control**: Fast (9 packages) vs Standard (17 packages) vs Comprehensive (47+ packages)
- **Paradigm-specific packages**: Analysis, Manuscript, and Package development workflows
...
Docker variants are pre-configured environments with specific base images, R packages, and system dependencies. Each variant is optimized for particular research workflows.
...
All variant definitions are stored in `templates/variant_examples.yaml`:
...
variants:
  analysis:
    enabled: true    # Enable this variant for the team
    # Full definition automatically pulled from variant_examples.yaml

  modeling:
    enabled: false   # Disable this variant (won't be built)
...
The `./add_variant.sh` script provides an interactive interface for variant selection:
...
- Multi-paradigm projects (analysis + modeling + publishing)
- Teaching environments with diverse package needs
- Production analysis environments

## Paradigm-Specific Package Selection

### Analysis Paradigm Packages
Optimized for data exploration and statistical analysis:

```r
analysis_packages <- c(
  # Data manipulation and visualization
  "tidyverse", "here", "janitor", "skimr", "naniar",
  # Analysis and modeling
  "broom", "infer", "moderndive",
  # Reporting and communication
  "targets", "rmarkdown", "flexdashboard", "DT", "plotly"
)
```

### Manuscript Paradigm Packages
...
manuscript_packages <- c(
  ...
  "rmarkdown", "bookdown", "papaja", "RefManageR",
  # Analysis support
  "here", "conflicted", "broom"
)
```

### Package Paradigm Packages
...
package_packages <- c(
  ...
  "pkgdown", "covr", "lintr", "styler",
  # Distribution and checking
  "rcmdcheck", "rhub", "spelling"
)
```
...
```yaml
development:
  # Default variant for team members joining
...
build:
  ...
  docker:
    ...
    parallel_builds: true       # Build variants in parallel
...
collaboration:
  documentation:
    ...
    include_variant_docs: true       # Document available variants
...
cd my-analysis
./add_variant.sh    # Add alpine_minimal for CI/CD testing
```
...
```bash
# View team's available variants
...
```bash
# 1. Copy and customize variant library
cp templates/variant_examples.yaml custom_variants.yaml
```

Edit `custom_variants.yaml` to add organization variants:
...
```bash
# Build with custom variant library
...
# Share custom variant library with organization
...
```bash
# Validate variant_examples.yaml syntax
./add_variant.sh --validate

# Check for missing or invalid variant references
zzcollab --variants-config config.yaml --dry-run

# Test variant definitions without building
./add_variant.sh --test-definitions
```
...
```bash
# Error: YAML configuration requires yq but it's not installed
...
```bash
# Error building variant 'geospatial': platform not supported
...
```bash
# User sets paradigm = "manuscript" but projects use "analysis"
...
# paradigm: "analysis" (from ./zzcollab.yaml) [OVERRIDING user config]
```

Edit the appropriate configuration file:
```bash
# To use user default, remove from project config
vim ./zzcollab.yaml  # Remove paradigm setting

# To set project-specific override, ensure it's intentional
echo "# Project uses analysis paradigm instead of user default" >> ./zzcollab.yaml
```
...
init_project("new-analysis")          # Uses configured team_name, paradigm, build_mode
...
init_project(
  ...
  paradigm = "manuscript"             # Override default paradigm
  # team_name, build_mode from config
)
...
init_project("temp-package-project")
...
- **Minimal variants**: Use 2-3 variants (minimal, analysis, alpine_minimal)
...
- **Paradigm alignment**: Team agrees on primary paradigm (analysis/manuscript)
...
- **Extensive variants**: Full variant library available to teams
- **Multiple paradigms**: Support analysis, manuscript, and package paradigms
- **Custom variant library**: Organization-specific variants in custom libraries
...
4. **Parallel builds**: Enable parallel variant building in configuration
...
```yaml
# Size-optimized variant strategy
variants:
  alpine_minimal:
...
ZZCOLLAB's configuration system provides powerful, flexible control over research environments while maintaining simplicity for common use cases. The multi-layered hierarchy allows individual preferences, team coordination, and organizational policies to coexist seamlessly.

Key takeaways:

1. **Start simple**: Use built-in build modes and standard variants for most projects
2. **Customize gradually**: Add custom variants and packages as needs become clear
3. **Coordinate with teams**: Use project configs to ensure team consistency
4. **Optimize for workflow**: Choose variants and packages that match your research paradigm
5. **Validate regularly**: Use configuration validation to catch issues early

The configuration system scales from solo researchers to large organizations, providing reproducible, customizable research environments that eliminate configuration drift and enable seamless collaboration.

For detailed technical documentation, see:
- `zzcollab --help` for command-line options
- `help(package = "zzcollab")` for R interface documentation
- `templates/variant_examples.yaml` for available variants
- `PARADIGM_GUIDE.md` for paradigm-specific guidance