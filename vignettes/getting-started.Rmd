...
The `--use-team-image` flag pulls a pre-built Docker image from Docker Hub
(if the team lead pushed one), which is faster than building from the Dockerfile.
Alternatively, team members can build from the Dockerfile using `make docker-build`.
Either way, all team members get identical Docker environments from the same
