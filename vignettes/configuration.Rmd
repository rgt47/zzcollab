...
- **✓ Flexible customization**: 14+ Docker profiles, custom package lists, build modes
- **✓ Hierarchical control**: User, project, and system-level configurations
- **✓ Single source of truth**: Centralized profile definitions prevent duplication
...
- **14+ predefined profiles** from minimal (~200MB Alpine) to comprehensive (~3.5GB full-featured)
...
Teams select which profiles to enable in their `config.yaml`:
...
Select profiles to add (e.g., 1,3,9):
```

Selected profiles are automatically added to your `config.yaml` with `enabled: true`.

### Manual Profile Management

You can also edit profiles directly in `config.yaml`:
...
profiles:
  analysis:
    enabled: true
    # Extend the base analysis profile
    additional_packages:
      - "pins"
      - "vetiver"
      - "plumber"
...
collaboration:
  documentation:
    ...
    include_profile_docs: true       # Document available profiles
...
zzcollab -p my-analysis

# Build and push team image (if sharing with others)
make docker-build
make docker-push-team

# Customize profiles for this specific project
...
zzcollab -t datasci-lab -p customer-churn-analysis
```

### Customize Team Profiles
...
# 2. Pull and use team Docker image
...
# Check team's profile configuration
cat config.yaml | grep -A 20 "build:"
```

### Working with Different Docker Profiles

The team's configuration file (`config.yaml`) determines which Docker profiles are available. If multiple profiles are enabled, team members can work with the single team image which includes the configured environment.
...
**Note**: The new architecture uses a single team image per project. If your team needs multiple specialized environments (e.g., analysis, modeling, publishing), consider creating separate projects or using the Docker profile system to customize the single team image.

## Advanced Custom Profile Workflow
...
Edit `custom_profiles.yaml` to add organization profiles:
...
build:
  ...
  profile_library: "custom_profiles.yaml"  # Use custom library

profiles:
  minimal:
    enabled: true
  org_finance:
    enabled: true    # Use custom finance profile
  org_research:
    enabled: true    # Use custom research profile
```

### Building and Sharing Custom Profiles
...
git add custom_profiles.yaml config.yaml
git commit -m "Add organization-specific profiles"
git push origin main
```
...
zzcollab --config get profiles.analysis.enabled
...
zzcollab --profiles-config config.yaml --dry-run
...
# Test Docker build with specific profile
zzcollab --profiles-config config.yaml --build-only minimal
```
...
init_project(
  ...
  profiles = c("minimal", "analysis", "alpine_minimal")
)
```
...
- **Minimal profiles**: Use 2-3 profiles (minimal, analysis, alpine_minimal)
...
- **Focused profiles**: 3-5 profiles covering team's research domains
...
- **Extensive profiles**: Full profile library available to teams
- **Complete research lifecycle**: All workflows supported (data analysis, manuscripts, packages)
- **Custom profile library**: Organization-specific profiles in custom libraries
...
build:
  profile_library: "/etc/zzcollab/org_profiles.yaml"
...
2. **Use Alpine profiles**: For CI/CD and testing workflows
3. **Separate concerns**: Use different profiles for different purposes
...
profiles:
  ...
  # Don't enable comprehensive profiles unless needed
```
...
- **Project configs**: Team settings (profiles, collaboration rules)
...
1. **Start simple**: Use built-in build modes and standard profiles for most projects
...
4. **Optimize for workflow**: Choose profiles and packages that match your research needs
