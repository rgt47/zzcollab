...
- **✓ Flexible customization**: 14+ Docker environments, custom package lists, build modes
- **✓ Hierarchical control**: User, project, and system-level configurations
- **✓ Single source of truth**: Centralized environment definitions prevent duplication
...
- **Interactive environment browser** for easy discovery and selection
...
Docker environments are pre-configured environments with specific base images, R packages, and system dependencies. Each environment is optimized for particular research workflows.
...
All environment definitions are stored in `templates/environments.yaml`:
...
Teams select which environments to enable in their `config.yaml`:

```yaml
environments:
  analysis:
    enabled: true    # Enable this environment for the team
    # Full definition automatically pulled from environments.yaml

  modeling:
    enabled: false   # Disable this environment (will not be built)
...
The `./add_environment.sh` script provides an interactive interface for environment selection:
...
Select environments to add (e.g., 1,3,9):
```

Selected environments are automatically added to your `config.yaml` with `enabled: true`.

### Manual Variant Management

You can also edit environments directly in `config.yaml`:
...
Add packages to existing environments:
...
Define completely custom environments:
...
```yaml
development:
  # Default environment for team members joining
...
build:
  ...
  docker:
    ...
    parallel_builds: true       # Build environments in parallel
...
collaboration:
  documentation:
    ...
    include_variant_docs: true       # Document available environments
...
# Customize environments for this specific project
cd my-analysis
./add_environment.sh    # Add alpine_minimal for CI/CD testing
```
...
```bash
# View team's available environments
...
```bash
# 1. Copy and customize environment library
cp templates/environments.yaml custom_variants.yaml
```

Edit `custom_variants.yaml` to add organization environments:

```yaml
# Add to existing environments
org_finance:
  base_image: "rocker/r-ver:latest"
...
build:
  ...
  environment_library: "custom_variants.yaml"  # Use custom library
...
```bash
# Build with custom environment library
...
# Share custom environment library with organization
git add custom_variants.yaml config.yaml
git commit -m "Add organization-specific environments"
git push origin main
```
...
```bash
# Validate environments.yaml syntax
./add_environment.sh --validate

# Check for missing or invalid environment references
zzcollab --variants-config config.yaml --dry-run

# Test environment definitions without building
./add_environment.sh --test-definitions
```
...
# Test Docker build with specific environment
...
```bash
# Error building environment 'geospatial': platform not supported
...
Or use ARM64-compatible environments:
```yaml
environments:
  analysis:
...
init_project(
  ...
  environments = c("minimal", "analysis", "alpine_minimal")
)
```
...
- **Minimal variants**: Use 2-3 environments (minimal, analysis, alpine_minimal)
...
- **Focused variants**: 3-5 environments covering team's research domains
...
- **Extensive variants**: Full environment library available to teams
- **Complete research lifecycle**: All workflows supported (data analysis, manuscripts, packages)
- **Custom environment library**: Organization-specific environments in custom libraries
...
build:
  environment_library: "/etc/zzcollab/org_variants.yaml"
...
4. **Parallel builds**: Enable parallel environment building in configuration
...
2. **Use Alpine environments**: For CI/CD and testing workflows
3. **Separate concerns**: Use different environments for different purposes

```yaml
# Size-optimized environment strategy
environments:
  alpine_minimal:
    enabled: true     # ~200MB for CI/CD
  analysis:
    enabled: true     # ~1.2GB for development
  # Don't enable comprehensive environments unless needed
```
...
- **Project configs**: Team settings (environments, collaboration rules)
...
1. **Start simple**: Use built-in build modes and standard environments for most projects
2. **Customize gradually**: Add custom environments and packages as needs become clear
3. **Coordinate with teams**: Use project configs to ensure team consistency
4. **Optimize for workflow**: Choose environments and packages that match your research needs
...
- `templates/environments.yaml` for available environments
