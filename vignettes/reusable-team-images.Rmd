...
zzcollab -i -t mylab -p baseimage -C -B rstudio -d ~/dotfiles

# Project 1: Use team image as base (~30 seconds)
zzcollab -t mylab -p study1 -I rstudio -d ~/dotfiles

# Project 2: Use team image as base (~30 seconds)
zzcollab -t mylab -p study2 -I rstudio -d ~/dotfiles

# Project 3: Use team image as base (~30 seconds)
zzcollab -t mylab -p study3 -I rstudio -d ~/dotfiles
...
zzcollab -i -t mylab -p baseimage -S -B rstudio -d ~/dotfiles

# Fast mode (9 packages, minimal overhead)
zzcollab -i -t mylab -p baseimage -F -B rstudio -d ~/dotfiles
```

**Multiple interface variants:**
```bash
# Shell + RStudio + Verse (publishing workflow)
zzcollab -i -t mylab -p baseimage -C -B all -d ~/dotfiles

# Just RStudio (most common)
zzcollab -i -t mylab -p baseimage -C -B rstudio -d ~/dotfiles

# Shell only (command-line work)
zzcollab -i -t mylab -p baseimage -C -B r-ver -d ~/dotfiles
```
...
**Important**: Notice we use `-I rstudio` (interface selection) instead of `-B rstudio` (base image building). The `-I` flag tells zzcollab to use an existing team image.
...
zzcollab -i -t genomicslab -p labbase -C -B rstudio -d ~/dotfiles

# Optionally add specialized variant
cd /path/to/zzcollab
./add_variant.sh
# Select: bioinformatics variant
...
cd cancer-study
zzcollab -t genomicslab -p cancer-study -I rstudio -d ~/dotfiles
...
cd rnaseq-analysis
zzcollab -t genomicslab -p rnaseq-analysis -I rstudio -d ~/dotfiles
make docker-rstudio

# Researcher 3: New variant calling project
mkdir variant-calling && cd variant-calling
zzcollab -t genomicslab -p variant-calling -I rstudio -d ~/dotfiles
make docker-rstudio
...
zzcollab -i -t datasci-firm -p q1-2025-base -C -B rstudio -d ~/dotfiles
...
zzcollab -t datasci-firm -p client-a-retail -I rstudio -d ~/dotfiles

# Client B: Financial modeling
mkdir client-b-finance && cd client-b-finance
zzcollab -t datasci-firm -p client-b-finance -I rstudio -d ~/dotfiles

# Client C: Healthcare dashboards
mkdir client-c-health && cd client-c-health
zzcollab -t datasci-firm -p client-c-health -I rstudio -d ~/dotfiles
...
zzcollab -i -t mylab -p base-2025-01 -C -B rstudio -d ~/dotfiles
...
# Option 2: Use cached images (don't pull if local copy exists)
docker images | grep mylab/baseimageecore
...
zzcollab -i -t mylab -p baseimage-test -C -B rstudio -d ~/dotfiles

# Test with sample project
mkdir test-project && cd test-project
zzcollab -t mylab -p test-project -I rstudio -d ~/dotfiles
make docker-rstudio
...
zzcollab -t mylab -p <project-name> -I rstudio -d ~/dotfiles
