...
│   ├── journal_versions/      # Journal-specific formats
│   └── review_responses/      # Peer review management
├── data/processed/            # Team-validated datasets
...
Morphometric data were obtained from the Palmer Archipelago penguin dataset [@Horst2020], consisting of measurements from adult penguins collected at three research stations (Biscoe, Dream, and Torgersen islands) during the 2007-2009 breeding seasons. The dataset includes bill length (mm), bill depth (mm), flipper length (mm), and body mass (g) measurements for *n* = `r nrow(morphology_data)` individuals across three congener species.

Data processing followed standardized protocols to ensure reproducibility and analytical consistency:

```{r data-processing, echo=TRUE, eval=TRUE}
# Load team-standardized dataset
...
```{r allometry-example, echo=TRUE, eval=TRUE}
# Demonstrate team allometry modeling approach
...
kable(format_morphometry_table(example_model),
      caption = "Example allometric model results")
```
