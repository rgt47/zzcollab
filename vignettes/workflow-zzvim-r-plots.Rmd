...
diagnostics run inside the container via the `languageserver` package. The
Docker image installs it by default (config key `languageserver`, default
`true`); your host editor (vim) bridges its LSP client into the container. No
host R install is required. For REPL-only workflows you can opt out with:
