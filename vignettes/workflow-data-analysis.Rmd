...
This vignette demonstrates the recommended workflow for reproducible data analysis using ZZCOLLAB, following the unified research compendium framework [@marwick2018] and best practices from the reproducible research literature [@wilson2017; @stodden2018].

**Key Philosophy**: Start with simple analysis, add complexity only as needed. The same project structure serves you from initial exploration through publication [@marwick2018].
...
- Collaborating with version control [@ram2013; @bryan2018]
...
ZZCOLLAB implements the unified research compendium framework [@marwick2018], which provides **one flexible structure for all research**—from simple analysis through manuscript writing to package distribution.

## Core Principle: Progressive Disclosure

**Start simple, add complexity organically, never rebuild** [@marwick2018].
...
**No migration needed**—the structure grows with your research [@marwick2018].
...
zzcollab config set profile-name "ubuntu_standard_analysis"
```
...
Following best practices for data documentation [@wilson2017], ZZCOLLAB provides comprehensive data documentation templates.
...
# Inside R: Install palmerpenguins
...
Following the rrtools approach [@marwick2018], start with analysis scripts in `analysis/scripts/`, then extract reusable functions to `R/` when patterns emerge.
...
# Exit container (auto-snapshot runs on R exit!)
exit
```

**Tip**: Use `make r` if you prefer to work directly in R. Use `make r` if you need shell access for running multiple scripts.

## Step 4: Extract Reusable Functions (Progressive Disclosure)

As you identify repeated patterns, extract them to `R/` directory for reuse and testing [@marwick2018; @wickham2015].
...
Testing validates computational correctness and prevents regressions [@wilson2017; @stodden2018].
...
Version control is essential for reproducible research [@ram2013; @bryan2018].
...
Following best practices for computational reproducibility [@stodden2018]:
...
# Enter container - auto-restore runs on R startup!
make r

# Inside R: packages are automatically restored from renv.lock
# Run analysis scripts
> source("analysis/scripts/01_prepare_data.R")
> source("analysis/scripts/02_explore_data.R")
> source("analysis/scripts/03_statistical_analysis.R")
> q()

# Or use shell for batch execution
make r
Rscript analysis/scripts/01_prepare_data.R
Rscript analysis/scripts/02_explore_data.R
Rscript analysis/scripts/03_statistical_analysis.R
...
**Auto-Restore**: When R starts, ZZCOLLAB automatically restores packages from `renv.lock` if they're missing. No manual `renv::restore()` required.

## Step 7: Validate Reproducibility

ZZCOLLAB provides automated reproducibility validation [@stodden2018].
...
For datasets >100MB, use data versioning [@wilson2017]:
...
# Custom composition
zzcollab -b rocker/r-ver --pkgs modeling
```
...
When ready to write up results, simply add `analysis/report/report.Rmd`:
...
**No restructuring required** - the unified compendium serves both analysis and manuscript [@marwick2018].

## Publishing as Blog Post

Want to share your analysis as a blog post? ZZCOLLAB projects integrate with Quarto blogs:

```bash
# Run the blog setup script to create symlink structure
modules/setup_symlinks.sh

# See vignette: "Blog Post Development with ZZCOLLAB"
```

The setup script creates a dual-symlink structure that lets Quarto find your `index.qmd` while you work in the familiar `analysis/report/` directory.
...
**No migration required** - add documentation and submit to CRAN [@wickham2015].

# Best Practices Summary

Following reproducible research best practices [@wilson2017; @stodden2018]:
