...
The post and its assets live together under `analysis/`, so figures, media,
and data are referenced with paths relative to `analysis/report/index.qmd`.
Quarto renders the document in place; `make docker-render-qmd` runs
`quarto render analysis/report/index.qmd` inside the container.

**Benefits**:

1. **No symlinks to maintain**: One canonical location for the post and assets
2. **Intuitive paths**: Reference assets relative to `analysis/report/`
3. **Reproducible render**: `make docker-render-qmd` builds in the container
...
inside the container. No symlinks are required.
...
Author the post at `analysis/report/index.qmd`. Asset paths are relative to
that file, so they begin with `../` to reach `analysis/figures/`,
