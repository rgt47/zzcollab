---
title: "ZZCOLLAB for R Users: Solo Developer Workflow"
...
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{ZZCOLLAB for R Users: Solo Developer Workflow}
...
# ZZCOLLAB for R Users: Solo Developer Workflow

This vignette is designed for R developers who want to use ZZCOLLAB's reproducible research capabilities **without needing to learn Docker or bash commands**. Everything is handled through R functions that work behind the scenes.

## Why ZZCOLLAB for R Users?

As an R developer, you're already comfortable with:
...
ZZCOLLAB brings you **Docker-powered reproducibility** with the same familiar R interface you already know.

**What you get:**
- 🔒 **Isolated environments** - Your projects never interfere with each other
- 🎯 **Reproducible research** - Exact same environment every time
- 🚀 **Team-ready projects** - Easy to share with collaborators
- 🔧 **No Docker knowledge required** - All handled through R functions
...
```{r eval=FALSE}
# Fast mode: ~15 essential packages (recommended for beginners)
set_config("build_mode", "fast")

# Standard mode: ~25 packages including tidyverse (recommended for most users) 
set_config("build_mode", "standard")

# Comprehensive mode: 35+ packages for complex analyses
...
ZZCOLLAB integrates seamlessly with Git through R functions:
...
For R users curious about the magic:
...
```{r eval=FALSE}
# Share projects with perfect reproducibility
...
ZZCOLLAB transforms Docker-based reproducibility into a familiar R experience:

✅ **R-native interface** - Functions work like any R package  
✅ **No Docker knowledge needed** - Everything handled behind the scenes  
✅ **Familiar development** - RStudio, R scripts, regular R packages  
✅ **Automatic reproducibility** - Perfect environments without extra work  
✅ **Team collaboration** - Share exact environments with single function calls  
✅ **Version control integration** - Git workflows through R functions  
...
Your research will be reproducible, shareable, and professional-grade, all through the R interface you already know and love.