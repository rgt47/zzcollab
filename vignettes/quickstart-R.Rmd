...
>   here driven through its R-function equivalents.
...
- **Auto-snapshot**: Packages captured in renv.lock when you exit containers (no
  manual `renv::snapshot()` needed)
- **Auto-restore**: Missing packages installed automatically when you start R (no manual `renv::restore()` needed)
- Both `add_package()` (R function) and `install.packages()` (standard R) work identically
