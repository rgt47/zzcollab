...
zzcollab -c set github-account "your-github-username"
```

### 1.3 Create Project Workspace

Create a new directory and initialize the zzcollab project:

```bash
mkdir mtcars-analysis
cd mtcars-analysis
zzcollab -t myteam -p mtcars-analysis -r analysis
```
...
Inside R, extract the first 50% of mtcars and save to the proper location:

```r
# Extract first half of mtcars (16 observations)
n_rows <- nrow(mtcars)
mtcars_subset <- mtcars[1:(n_rows %/% 2), ]

# Verify the subset
dim(mtcars_subset)
# [1] 16 11

# Save to raw_data directory (read-only after this point)
saveRDS(mtcars_subset, "analysis/data/raw_data/mtcars_subset.rds")
...
writeLines(
  c(
    ...
    "",
    "## Source",
    "First 50% (16 observations) of the base R mtcars dataset.",
    "",
    "## Variables",
    ...
    "",
    "## Notes",
    "This subset is used for demonstration purposes.",
    "Original dataset: Henderson and Velleman (1981)."
  ),
...
```r
# 01_prepare_data.R
# Purpose: Load and prepare mtcars subset for analysis
...
if (sys.nframe() == 0) {
  prepare_mtcars_data(
    input_path = "analysis/data/raw_data/mtcars_subset.rds",
...
test_that("raw data file exists and is readable", {
  raw_data_path <- "analysis/data/raw_data/mtcars_subset.rds"
...
test_that("raw data has expected dimensions", {
  data <- readRDS("analysis/data/raw_data/mtcars_subset.rds")
...
```bash
git remote add origin git@github.com:myteam/mtcars-analysis.git
git branch -M main
git push -u origin main
```

### 5.3 Build and Push Team Docker Image
...
Developer 2 clones the project and sets up the environment:
...
The `-u` flag pulls the team's Docker image.
...
| Push team image | `make docker-push-team` |
| Use team image | `zzcollab -u` |
