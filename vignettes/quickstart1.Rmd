...
Four prompts, each pre-filled from your user config. Press Enter to accept
the default or type to override. Changes are written only to
...
documents project-specific decisions and allows collaborators to reproduce
...
```bash
# Create new projects with specific profiles
mkdir genomics-study && cd genomics-study
zzcollab docker --base-image rocker/shiny   # Shiny Server via the rocker/shiny base image
...
For machine learning, install packages such as `tidymodels` via renv. For
LaTeX/Quarto manuscripts use `zzcollab docker --base-image rocker/verse`, and
for Shiny applications use `zzcollab docker --base-image rocker/shiny`.
...
# The analysis profile builds on rocker/tidyverse. For LaTeX/Quarto
# manuscripts use zzcollab docker --base-image rocker/verse.
...
# Change base image on existing project
zzcollab docker --base-image rocker/verse   # LaTeX/Quarto via the rocker/verse base image

# Create with a different base image
...
---
*Rendered on 2026-05-29 at 06:45 PDT.*<br>
