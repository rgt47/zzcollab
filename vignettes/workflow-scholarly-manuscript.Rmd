...
**Key Philosophy**: Use the **same structure** for manuscript writing as data analysis. No separate "manuscript paradigm"—simply add `analysis/report/report.Rmd` when ready to write [@marwick2018].
...
**ZZCOLLAB approach** [@marwick2018; @gentleman2007]: **Same project, just add report.Rmd**.
...
2. **Week 9+**: Add `analysis/report/report.Rmd` to write up results
...
1. **Day 1**: Create `analysis/report/report.Rmd` with introduction
...
**Key insight**: This is **exactly the same structure** as data analysis. The only difference is you'll add content to `analysis/report/` [@marwick2018].
...
**Tip**: Use `make r` if you prefer to work directly in R. Use `make r` for shell access when running batch scripts.
...
Manuscript source: `analysis/report/report.Rmd`
...
wget https://raw.githubusercontent.com/citation-style-language/styles/master/ecology-letters.csl \
  -O analysis/report/ecology-letters.csl
...
R
> rmarkdown::render("analysis/report/report.Rmd", output_format = "pdf_document")
> quit()

exit
```

Output: `analysis/report/paper.pdf`
...
R
> rmarkdown::render("analysis/report/report.Rmd", output_format = "word_document")
> quit()

exit
```

Output: `analysis/report/paper.docx`
...
R
> rmarkdown::render("analysis/report/report.Rmd", output_format = "html_document")
> quit()

exit
```

Output: `analysis/report/paper.html`
...
R
> rmarkdown::render("analysis/report/report.Rmd", output_format = "all")
> quit()
...
Create `analysis/report/supplementary.Rmd`:
...
manuscript-pdf:
	...
	docker run --rm -v "$(PWD):/project" -w /project $(DOCKER_IMAGE) \
		Rscript -e "rmarkdown::render('analysis/report/report.Rmd', output_format = 'pdf_document')"

manuscript-word:
	@echo "Rendering manuscript to Word..."
	docker run --rm -v "$(PWD):/project" -w /project $(DOCKER_IMAGE) \
		Rscript -e "rmarkdown::render('analysis/report/report.Rmd', output_format = 'word_document')"

manuscript-html:
	@echo "Rendering manuscript to HTML..."
	docker run --rm -v "$(PWD):/project" -w /project $(DOCKER_IMAGE) \
		Rscript -e "rmarkdown::render('analysis/report/report.Rmd', output_format = 'html_document')"
...
supplementary:
	...
	docker run --rm -v "$(PWD):/project" -w /project $(DOCKER_IMAGE) \
		Rscript -e "rmarkdown::render('analysis/report/supplementary.Rmd', output_format = 'all')"
```
...
git add analysis/report/supplementary.Rmd
git add analysis/report/references.bib
git add analysis/report/*.csl
  ...
  render-manuscript:
    ...
    steps:
      ...
      - name: Render supplementary materials
        run: |
          Rscript -e "rmarkdown::render('analysis/report/supplementary.Rmd', output_format = 'pdf_document')"
      ...
      - name: Upload PDF artifacts
        ...
        with:
          ...
          path: |
            ...
            analysis/report/supplementary.pdf
      ...
      - name: Upload Word artifact
        ...
        with:
          ...
          path: analysis/report/paper.docx
```
...
Quarto (https://quarto.org) is the successor to R Markdown [@xie2018].
...
Create `analysis/report/paper.qmd`:
...
(Same content as report.Rmd, but with enhanced features)
...
quarto render analysis/report/paper.qmd --to all
...
✅ **Source in version control** - report.Rmd tracked with git
...
│       ├── report.Rmd           # ADD: Manuscript source
...
Want to share your manuscript findings as a blog post? ZZCOLLAB projects integrate with Quarto blogs:
