...
Allometric scaling relationships provide insights into ecological and
evolutionary processes [@schmidt-nielsen1984]. Penguin bill morphology
varies across species, suggesting differential selection pressures
[@gorman2014].

**Research question**: How do bill dimensions scale with body mass across
penguin species?
...
We analyzed morphological measurements from `r nrow(penguins)` penguins
of three species collected at Palmer Station, Antarctica [@horst2020].

```{r data-exploration}
# Initial exploration - inline for now
penguins |>
  filter(!is.na(body_mass_g)) |>
  group_by(species) |>
  summarise(
...
We fit linear models relating log-transformed bill dimensions to
log-transformed body mass, with species as a covariate.

```{r models}
# Inline for now - will extract if reused
...
[To be written as analysis progresses]
...
```{r scaling-analysis}
# Extract slopes for each species
library(broom)
...
slopes |>
  knitr::kable(
    digits = 3,
    caption = "Allometric slopes by species"
  )
```

Bill length showed near-isometric scaling with body mass
(slope = `r round(mean(slopes$slope), 2)`; Table 2).
````
...
````markdown
```{r load-results}
# Load pre-computed results (run 01_fit_models.R first)
