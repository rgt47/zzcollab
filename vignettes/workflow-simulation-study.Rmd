...
Then add your setup chunk and content. Document your simulation parameters
in a table:
...
This project uses tinytest. Create `inst/tinytest/test_simulate_data.R` with
...
Once the simulation packages (`furrr`, `geepack`, `lme4`, `progressr`, and the
rest) are installed, lock them into the manifest before committing:
...
`renv.lock` pins the exact package versions, while `DESCRIPTION` declares
dependency roles (packages used by code in `R/` go in `Imports`; packages used
only by `analysis/` go in `Suggests`).
