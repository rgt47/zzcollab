...
- **Workflow**: Function Development ↔ Manuscript Writing ↔ Testing ↔ Documentation ↔ Submission
...
#
# 🔬 SPECIALIZED DOMAINS (if applicable)
...
```
research-paper/
├── DESCRIPTION                 # Package metadata & dependencies (REQUIRED)
├── NAMESPACE                   # Exported functions (REQUIRED)
├── .Rbuildignore              # Exclude manuscript files from package build
├── R/                          # Analysis functions (EDITABLE)
│   ├── data_processing.R       # Data cleaning and preparation functions
│   ├── statistical_analysis.R  # Statistical models and tests
│   ├── visualization.R         # Publication-ready figure functions
│   └── manuscript_utils.R      # Manuscript-specific utilities
├── man/                        # Generated function documentation
│   ├── prepare_penguin_data.Rd
│   ├── analyze_species_differences.Rd
│   └── create_species_plot.Rd
├── tests/testthat/             # Function tests (EDITABLE)
│   ├── test-data_processing.R
│   ├── test-statistical_analysis.R
│   ├── test-visualization.R
│   └── test-manuscript_utils.R
├── manuscript/                 # Academic writing infrastructure (EDITABLE)
│   ├── paper.Rmd              # Main manuscript (papaja format)
│   ├── supplementary.Rmd      # Supplementary materials
│   ├── references.bib         # Bibliography
│   └── journal_templates/     # Journal-specific formatting
├── analysis/reproduce/         # Reproducibility scripts (EDITABLE)
│   ├── 01_data_preparation.R   # Data processing workflow
│   ├── 02_statistical_analysis.R # Statistical analysis pipeline
│   ├── 03_figures_tables.R     # Generate all manuscript outputs
│   └── 04_manuscript_render.R  # Render all manuscript formats
...
│   ├── raw_data/              # Original, immutable data files
│   ├── derived_data/          # Processed, analysis-ready data
│   └── README.md              # Comprehensive data documentation
├── vignettes/                 # Package vignettes (optional)
│   └── analysis_methods.Rmd  # Methodological documentation
├── inst/                      # Package installation files
│   └── examples/              # Reproducible examples
├── renv.lock                  # Environment snapshot
└── .github/workflows/         # Automated validation
    ├── R-CMD-check.yaml       # R package validation
    └── render-manuscript.yaml # Manuscript compilation
```
...
1. **GitHub Actions automatically**:
   - ✅ Renders complete manuscript (paper.Rmd → PDF)
   - ✅ Runs all function tests
   - ✅ Executes reproducibility scripts
   - ✅ Generates all figures and tables
   - ✅ Validates bibliography and citations

2. **If manuscript compilation fails**:
   - ❌ Build log shows specific LaTeX/rendering errors
   - ❌ Email notification with error details
   - ✅ Fix errors and push again for re-validation
...
We analyzed morphological measurements from `r nrow(penguin_data)` penguins representing three species: Adelie (n = `r summary_stats$n[summary_stats$species == "Adelie"]`), Chinstrap (n = `r summary_stats$n[summary_stats$species == "Chinstrap"]`), and Gentoo (n = `r summary_stats$n[summary_stats$species == "Gentoo"]`) collected between 2007-2009 in the Palmer Archipelago, Antarctica.
...
Species differences were tested using one-way ANOVA for each morphological variable, with Tukey's HSD post-hoc tests for pairwise comparisons. Effect sizes were calculated using eta-squared ($\eta^2$). Statistical significance was set at α = 0.05.

```{r bill-length-analysis, include = FALSE}
...
@article{fraser2019ecological,
  title={The ecological basis for inferring population trends of Adélie penguins from their prey},
...
@article{ainley2002adelie,
  title={The Adélie penguin: bellwether of climate change},
  author={Ainley, David},
