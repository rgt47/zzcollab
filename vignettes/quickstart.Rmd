...
# Initialize configuration (creates ~/.zzcollab/config.yaml)
init_config()
...
- Change defaults anytime with `set_config()`
- Configuration stored in `~/.zzcollab/config.yaml`
...
zzcollab  # Uses bioinformatics profile from config
make docker-build
...
```r
# Option 1: Set team config once (in R)
library(zzcollab)
