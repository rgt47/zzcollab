...
- No need to specify `-t`, `-F`, `-D`, or `--github` flags repeatedly
...
# --github: from auto-github config (true)
...
2. `zzcollab`: Creates GitHub repository automatically (if auto-github enabled)
3. `zzcollab`: Sets up CI/CD workflows for automated testing
4. `zzcollab`: Initializes renv for dependency tracking
5. `make docker-build`: Builds Docker image with R + essential packages
...
**Optional: Choose your own renv mode:**

```bash
# Team members can optionally specify their preferred renv mode
# The team Docker image (bioinformatics) is pulled automatically

zzcollab --use-team-image -M  # Minimal renv (3 packages)
zzcollab --use-team-image -F  # Fast renv (9 packages)
zzcollab --use-team-image -S  # Standard renv (17 packages, default)
zzcollab --use-team-image -C  # Comprehensive renv (47 packages)
```
...
4. Initializes renv with mode you chose (minimal, fast, standard, or comprehensive)
...
- ✓ Can choose renv mode independently (use `-M/-F/-S/-C`)
...
mkdir penguin-bills && cd penguin-bills
zzcollab  # Complete setup without GitHub
gh repo create rgt47/penguin-bills --private --source=. --remote=origin --push
```
...
zzcollab --config set profile-name "bioinformatics"
zzcollab --config set renv-mode "fast"
mkdir genomics-analysis && cd genomics-analysis
zzcollab
make docker-build
```
...
zzcollab --use-team-image -M

# Bob uses comprehensive renv (all packages)
zzcollab --use-team-image -C
```

**Result**:
- Both get identical Bioconductor Docker environment (Layer 1) via `--use-team-image`
