...
- **profile-name**: Controls Docker environment (team/shared)
  - 11 predefined profiles available
...
- Packages are added via `install.packages()` inside the container as needed
- Packages automatically captured in renv.lock when you exit the container
...
ZZCOLLAB provides 11 predefined profiles for specialized environments. Use the `-r` flag to specify a profile:
...
zzcollab --list-profiles  # Show all 11 predefined profiles
...
- **Docker Profile** (`-r` flag): Controls the base Docker image and pre-installed packages
  ...
  - 11 predefined profiles: minimal, analysis, modeling, bioinformatics, geospatial, and more
  - Specify per project with `-r` flag (e.g., `zzcollab -r bioinformatics`)

- **Package Management**: Packages are added dynamically as needed
  - Use `install.packages("package-name")` inside the container
...
## Step 4: Add Research Data and Processing Pipeline (4 minutes)
...
cat > analysis/data/raw_data/penguins_raw.csv << 'EOF'
...
cat > analysis/data/raw_data/README.md << 'EOF'
...
cat > analysis/scripts/01_process_data.R << 'EOF'
...
# Document the processed data
cat > analysis/data/derived_data/README.md << 'EOF'
...
```bash
# Create test file for data utilities
cat > tests/testthat/test-data_utils.R << 'EOF'
# Source functions
...
## Step 5: Create Analysis Report (2 minutes)

Now create a report that uses the processed data and functions we created in Step 4.

### Create Visualization Function

Add a plotting function to `R/bill_analysis.R`:

```bash
cat > R/bill_analysis.R << 'EOF'
...
### Optional: GUI Support for Interactive Graphics (Advanced)
...
For the rest of this tutorial, we save plots to files, which works in all environments including CI/CD.
...
**11 Available Profiles:**
- **minimal** - Essential development packages
- **analysis** - Data analysis with tidyverse
- **modeling** - Machine learning environment
- **bioinformatics** - Bioconductor genomics
- **geospatial** - GDAL/PROJ mapping tools
- **publishing** - LaTeX/Quarto publishing
- **shiny** - Shiny server for web apps
- **shiny_verse** - Shiny with tidyverse
- **alpine_minimal** - Ultra-lightweight Alpine
- **alpine_analysis** - Lightweight Alpine with tidyverse
- **hpc_alpine** - High-performance computing on Alpine
...
**Profiles** (`-r` flag):
- Complete Docker environment shortcuts (11 predefined)
...
- The renv.lock file accumulates packages from all team members automatically on container exit
