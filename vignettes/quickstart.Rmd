...
## Step 2: Create Team Images and Project (4-5 minutes)

ZZCOLLAB uses a **two-step workflow** for team projects:

**Step 2a: Create Team Docker Images (2-3 minutes)**

First, create and push team Docker images to Docker Hub:

```bash
# Create team Docker images (builds and pushes to Docker Hub)
# All parameters come from Step 1 config (team-name, build-mode, dotfiles-dir)
zzcollab -i -p penguin-bills -y

# Flags explained:
# -i: Initialize team setup (creates Docker images only)
# -p: Project name
...
2. Builds Docker team images with R + essential packages
3. Pushes images to Docker Hub (e.g., `rgt47/penguin-bills_core-minimal:latest`)
4. Creates marker file `.zzcollab_team_setup` with team information
5. Creates basic configuration files (`config.yaml`, `profiles.yaml`)

**Step 2b: Complete Project Setup (1-2 minutes)**

After team images are created, complete the full project structure:

```bash
# Navigate to project directory
cd penguin-bills

# Complete full project setup (uses team images as base)
zzcollab -y

# Add -G flag if auto-github is not enabled in config:
# zzcollab -y -G   # Creates GitHub repository automatically
```

**What Step 2b does:**
1. Detects `.zzcollab_team_setup` marker (knows team images exist)
2. Creates personal Dockerfile using team image as base (`FROM rgt47/penguin-bills_core-minimal:latest`)
3. Creates complete project structure (R/, analysis/, tests/, .github/, etc.)
4. Creates GitHub repository (if -G flag or auto-github config enabled)
5. Sets up CI/CD workflows for automated testing
6. Initializes renv for dependency tracking

**Why Two Steps?**

This separation allows:
- **Team leads** to create and share team images once
- **Team members** to join later using those images (skip Step 2a)
- **Faster setup** for team members (no Docker image building)
- **Consistency** across entire team (everyone uses same base image)

**Default Docker Profiles Created (Step 2a):**
- `minimal` profile: Essential development packages (~800MB, 3 packages)

**Alternative: Interactive Profile Selection**

If you want to customize which Docker profiles to build (bioinformatics,
geospatial, Alpine, etc.), run the interactive profile manager after Step 2a:

```bash
# After Step 2a (team images created)
cd penguin-bills

# Browse and select from 14+ specialized profiles
./add_profile.sh

# Edit config.yaml to enable selected profiles
vim config.yaml

# Rebuild team images with selected profiles
zzcollab --profiles-config config.yaml

# Then complete Step 2b
zzcollab -y -G
```
...
Share your private repository with team members:

```bash
# Team Lead: Add collaborator
gh repo edit --add-collaborator username
```

**Team Member Joining Workflow:**

Team members clone the repository and use the existing team images:
...
# Complete personal setup (skips Step 2a - uses existing team images)
# The .zzcollab_team_setup marker tells zzcollab to use team images
zzcollab -y
...
1. Detects `.zzcollab_team_setup` marker in repository
2. Pulls team image `rgt47/penguin-bills_core-minimal:latest` from Docker Hub
...
5. Identical environment to team lead and all other team members
...
- **Step 2a** (Create team images): 2-3 minutes
- **Step 2b** (Complete project setup): 1-2 minutes
...
**Total**: ~11-13 minutes for complete 5-level reproducible analysis
(~15 minutes with optional Step 8)

**Team Member Time** (joining existing project): ~7-9 minutes
- Skips Step 1 (uses team config) and Step 2a (uses existing team images)
- Only needs Steps 2b-8
