...
- No need to specify `-t` or `-D` flags repeatedly
- Change defaults anytime with `zzcollab --config set`

**Configuration Options Explained:**

- **profile-name**: Controls Docker environment specification
  - Options: minimal, rstudio, analysis, bioinformatics, geospatial, etc.
  - See full list: `zzcollab --list-profiles`
...
zzcollab --list-profiles  # All predefined Docker profiles
...
- **Docker Profile** (`--profile-name`): Controls the base Docker image and pre-installed packages
...
- **Package Management**: Packages are added dynamically as needed
  - Use `renv::install("package-name")` inside the container
...
### Optional: GUI Support for Interactive Graphics

If you want to view R plots interactively (instead of saving to PNG files), you can use X11 forwarding with the `docker-zsh-gui` target. This enables GUI applications like R graphics windows and terminal emulators.

**One-time XQuartz setup** (macOS only):

```bash
# Install XQuartz (if not already installed)
brew install --cask xquartz

# Launch XQuartz and configure
open -a XQuartz
# In XQuartz: Preferences > Security > Enable "Allow connections from network clients"
# Restart XQuartz after enabling this setting

# Allow localhost connections (run this each time you restart your Mac)
export DISPLAY=:0
...
```bash
# Instead of make docker-zsh, use:
make docker-zsh-gui
```
...
R
plot(1:10, 1:10)  # Graphics window should appear on your Mac
```

**When to use GUI support:**
- ✓ Interactive plot exploration and refinement
- ✓ Using GUI terminal emulators (kitty)
- ✓ Quick visualization during analysis development
- ✗ Not needed for production scripts that save plots to files
- ✗ Not available in CI/CD environments (use file output there)

For the rest of this tutorial, we'll use `make docker-zsh` (non-GUI) and save plots to files, which works in all environments including CI/CD.
...
Add this content to `analysis/paper/paper.Rmd`:
...
# Back at shell: Validate dependencies (thorough approach)
Rscript validate_package_environment.R --fix
```

**Best Practice: Comprehensive Dependency Validation**

After completing your work in the container, run `validate_package_environment.R` once to comprehensively validate all dependencies. This is a thorough version of `renv::snapshot()` that:
- Scans all R code for package references
- Validates package names against CRAN/Bioconductor/GitHub (catches typos!)
- Updates DESCRIPTION file (adds missing packages, removes invalid ones)
- Identifies unused packages for cleanup
- Then calls `renv::snapshot()` to update renv.lock

This comprehensive approach ensures consistency between your code, DESCRIPTION, and renv.lock before committing.
...
## Step 8: Verify Report Rendering CI/CD (Optional, 30 seconds)
...
Create a team project and share the Docker specification:
...
# Create team project structure (generates Dockerfile with profile)
...
# Optional: Build and push team image to Docker Hub for faster team setup
make docker-build
...
mkdir study && cd study
zzcollab -t genomicslab -p study -b bioconductor/bioconductor_docker --libs bioinfo --pkgs bioinfo

# Commit Dockerfile to GitHub
git add .
git commit -m "Initial project setup with custom bioinfo composition"
git push -u origin main

# Optional: Build and push image for faster team setup
make docker-build
make docker-push-team
```

**Team Lead Responsibilities:**
1. Creates complete project structure with chosen Docker profile (generates Dockerfile)
2. Commits and pushes Dockerfile to GitHub repository
3. Adds collaborators to GitHub repository
4. Optionally: Builds and pushes Docker image to Docker Hub for faster team member setup

**Profile Selection for Teams:**
- Team lead chooses Docker profile (bioinformatics, geospatial, etc.) which generates the Dockerfile
- All team members get identical Docker environment from the Dockerfile (either by building it or pulling pre-built image)
- Team members add packages dynamically using `renv::install()` as needed
...
```bash
# Clone the repository (gets the Dockerfile)
git clone https://github.com/genomicslab/study.git
cd study

# Option 1: Pull pre-built image (faster, if team lead pushed one)
zzcollab --use-team-image
make docker-zsh

# Option 2: Build from Dockerfile (slower, works without Docker Hub)
make docker-build
make docker-zsh
```

**What happens for team members:**
1. Clone gets the Dockerfile committed by team lead
2. Either pull pre-built image (`--use-team-image`) OR build from Dockerfile (`make docker-build`)
3. Everyone gets identical Docker environment from the same Dockerfile specification
4. Add packages to analysis via `renv::install()` inside the container

**Adding Packages to Analysis:**

Team members can add R packages to the analysis (updates renv.lock and DESCRIPTION):

```bash
# Enter container
make docker-zsh

# Inside container (R prompt): Add packages for your analysis
library(devtools)  # or whatever packages you're working with
renv::install("tidymodels")
# ... do your analysis work ...
quit()

# Exit container - back at shell
exit
...
**Why validate_package_environment.R?**
- Scans code to find all package references (not just what you manually installed)
- Validates package names (catches typos before they break CI/CD)
- Updates DESCRIPTION file automatically
- Then updates renv.lock via `renv::snapshot()`
- Ensures consistency across code, DESCRIPTION, and renv.lock
- Run once per session after completing your work

**Team member permissions:**
- ✗ Cannot modify Docker image specification (Dockerfile, profile, base image, system libraries)
- ✓ Can add R packages to analysis via `renv::install()` (updates renv.lock only)
...
**Package Management:**
- Packages are added dynamically using `renv::install()` inside containers
...
- **Controlled by**: `renv::install()` inside containers
...
# Inside container (R prompt) - Alice adds modeling packages
renv::install("tidymodels")
# ... Alice does her work ...
quit()
...
renv::install("tidymodels")
# ... Alice does her modeling work ...
quit()

# Back at shell - exit container
exit

# Validate dependencies (scans all code, updates DESCRIPTION and renv.lock)
Rscript validate_package_environment.R --fix
...
# Inside container (R prompt):
renv::install("sf")
# ... Bob does his geospatial work ...
quit()

# Back at shell - exit container
exit

# Validate dependencies (renv.lock now has tidymodels + sf)
Rscript validate_package_environment.R --fix
