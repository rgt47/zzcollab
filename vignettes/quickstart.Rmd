...
If you want a specialized environment (bioinformatics, geospatial, etc.),
use the `--profile-name` flag:

```bash
# Bioinformatics environment with Bioconductor
mkdir genomics-study && cd genomics-study
zzcollab --profile-name bioinformatics

# Geospatial analysis with GDAL/PROJ/sf
mkdir spatial-analysis && cd spatial-analysis
zzcollab --profile-name geospatial

# Ultra-lightweight Alpine for CI/CD
mkdir alpine-project && cd alpine-project
...
```bash
# Create team project with specific profile
...
1. Builds team Docker images and pushes to Docker Hub
2. Creates complete project structure
3. Team members can join using those shared images
...
# Join team (uses existing team images from Docker Hub)
...
2. Creates personal Dockerfile using team image as base
3. Builds personal development image (fast - layers on top of team image)
4. Identical environment to team lead and all other team members

### Adding Packages to Team Environment

Team members can add additional R packages to their personal layer:
...
- ✓ Can add R packages (use `--pkgs`)
- ✓ Can select different team variants (use `--tag`)
...
- Sets of R packages for specific workflows

**Build Modes** (`-M`, `-F`, `-S`, `-C`):
- Control which packages are installed in renv (personal choice)
