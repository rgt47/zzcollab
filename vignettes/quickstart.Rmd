...
# Save dependencies (thorough validation approach)
Rscript validate_package_environment.R --fix
```

**Best Practice: Comprehensive Dependency Validation**

`validate_package_environment.R` is a thorough version of `renv::snapshot()` that:
...
# Inside container: Add packages for your analysis
renv::install("tidymodels")

# Validate and update dependencies (recommended)
Rscript validate_package_environment.R --fix
quit()
...
- Scans code to find all package references (not just what you installed)
...
# STEP 1: Run tests to ensure nothing broke
make docker-test

# STEP 2: Commit if tests passed
git add renv.lock DESCRIPTION
git commit -m "Add tidymodels for modeling workflow"
git push
...
# Inside container:
renv::install("sf")
Rscript validate_package_environment.R --fix  # renv.lock now has tidymodels + sf
quit()
