...
- Dotfiles directory (optional) at `~/dotfiles`

**Important Note About Dotfiles:**

ZZCOLLAB supports **two dotfiles modes**:

**Option 1: Files WITH dots (use `-d` flag) - RECOMMENDED**
```bash
~/dotfiles/
  .vimrc        # WITH leading dot
  .tmux.conf    # WITH leading dot
  .gitconfig    # WITH leading dot
  .zshrc        # WITH leading dot
```

**Option 2: Files WITHOUT dots (use `-D` flag)**
```bash
~/dotfiles/
  vimrc         # WITHOUT leading dot
  tmux.conf     # WITHOUT leading dot
  gitconfig     # WITHOUT leading dot
  zshrc         # WITHOUT leading dot
```

**How it works:**
- `-d ~/dotfiles`: Copies files WITH dots as-is (`.vimrc` → `.vimrc` in container)
- `-D ~/dotfiles`: Adds dots during copy (`vimrc` → `.vimrc` in container)

**Note**: All dotfiles are now OPTIONAL (wildcards used). Docker builds succeed even if dotfiles are missing.
...
# Note: renv::snapshot() is NO LONGER NEEDED!
# It will run automatically when you exit the container
```
...
4. Add packages as needed using `renv::install()` inside the container

**Environment Consistency:**
- **Docker environment**: Identical for all team members (from team image via `--use-team-image`)
- **Package management**: Add packages dynamically with `renv::install()` as needed
...
- ✗ Cannot change Docker profile (use `--profile-name`)
- ✗ Cannot change base image (use `-b`)
- ✗ Cannot add system libraries (use `--libs`)
- ✓ Can add R packages to Docker (use `--pkgs`)
- ✓ Can add packages to renv.lock (use `renv::install()` in container)
