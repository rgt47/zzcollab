...
zzcollab -i -t rgt47 -p penguin-bills -F -D ~/dotfiles
...
# -D ~/dotfiles: Include dotfiles from ~/dotfiles directory
#    Note: Files should be named WITHOUT leading dots
#    (vimrc, tmux.conf, gitconfig, zshrc)
#    ZZCOLLAB adds the dots automatically when copying to container
```
...
# Note: Most parameters inferred from Step 2a files (DESCRIPTION, config.yaml)
zzcollab -I shell -G
...
#
# These are automatically detected from Step 2a:
...
2. Creates GitHub repository (public by default)
...
├── .github/
│   └── workflows/
│       └── r-package.yml        # Automated validation
├── R/
│   └── bill_analysis.R          # Tested function
├── scripts/
│   └── 01_bill_analysis.R       # Analysis script
├── tests/
│   └── testthat/
│       └── test-bill_analysis.R # Unit tests
├── figures/
│   └── bill_scatter.png         # Output plot
...
```bash
# If -G flag fails, create repository manually
cd penguin-bills
gh repo create rgt47/penguin-bills --private --source=. --remote=origin --push

# Or skip -G flag in Step 2b and create later
...
# Clean rebuild (two-step process)
docker system prune
rm -rf penguin-bills  # Remove failed directory
zzcollab -i -t rgt47 -p penguin-bills -F -D ~/dotfiles
cd penguin-bills
zzcollab -I shell -G  # Parameters inferred from Step 2a files
```
...
**Total**: ~9-10 minutes for complete 5-level reproducible analysis
