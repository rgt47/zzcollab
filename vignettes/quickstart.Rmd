...
```bash
# Create project directory and initialize with Docker + GitHub
mkdir penguin-bills && cd penguin-bills
zzc docker analysis github

# Or step by step:
# zzc init                    # Create rrtools structure
# zzc docker analysis         # Add Docker with analysis profile
...
**What `zzc docker analysis github` does:**
...
5. Creates private GitHub repository and pushes

**Using Different Docker Profiles:**

Profile names can be used directly as commands:

```bash
# Profiles work standalone or after docker command
mkdir genomics-study && cd genomics-study
zzc docker shiny github        # Docker + shiny + GitHub in one command

mkdir spatial-analysis && cd spatial-analysis
zzc docker publishing          # Docker with publishing profile (LaTeX)
...
zzc list libs                  # System library bundles
zzc list pkgs                  # R package bundles
```
...
```bash
# Create team project with Docker + GitHub in one command
mkdir study && cd study
zzc docker analysis github

# Or with a specialized profile
mkdir genomics && cd genomics
zzc docker shiny github --public
...
1. Creates project with `zzc docker <profile> github`
2. Builds and pushes Docker image with `zzc dockerhub`
...
```bash
# Profile names work as standalone commands or after docker
mkdir my-project && cd my-project
zzc docker analysis              # Docker with analysis profile
...
zzc docker shiny github          # Docker + shiny + GitHub
...
- Examples: `minimal`, `analysis`, `publishing`, `shiny`
- Usage: `zzc docker analysis` or `zzc analysis` (to change existing)
...
- **Controlled by**: Profile selection (e.g., `zzc docker analysis`)
...
```bash
mkdir study && cd study
zzc docker analysis github       # Create project + Docker + GitHub
make docker-build
zzc dockerhub                    # Push to DockerHub
```
