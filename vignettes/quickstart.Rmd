...
Your analysis is now fully reproducible thanks to four essential components:
...
### Additional Elements for Complete Reproducibility

**Data** (when not from packages):
- **Raw data**: `analysis/data/raw_data/` (version controlled or documented acquisition)
- **Derived data**: `analysis/data/derived_data/` (with processing scripts)
- **Data documentation**: `data/README.md` (data dictionary, sources, processing steps)

**Environment Variables** (set in Dockerfile):
- **Locale settings**: `LANG`, `LC_ALL` (affect sorting, number formatting)
- **Parallelization**: `OMP_NUM_THREADS` (for reproducible parallel processing)
- **Timezone**: `TZ` (for date/time operations)
...
**Key Insight**:
- **Dockerfile** = Environment foundation (R version, system libraries, locale)
...
All four are version controlled in Git, ensuring anyone can reproduce your exact computational environment and results.

### Reproducibility Checklist

When preparing analysis for publication or sharing:

- ✅ **Dockerfile specifies**: R version, system dependencies, environment variables
- ✅ **renv.lock committed**: Contains all packages from all contributors
- ✅ **.Rprofile version controlled**: Critical options documented
- ✅ **Random seeds explicit**: All stochastic operations use `set.seed()`
- ✅ **Environment variables set**: Locale (LANG, LC_ALL), timezone (TZ), parallelization
- ✅ **Data documented**: Raw data sources, processing scripts, data dictionary
- ✅ **CI/CD validates**: `check_rprofile_options.R` runs automatically
