...
## Step 2: Create Project with All 5 Levels (3-4 minutes)

Create a fully reproducible project in two steps:

### Step 2a: Create Team Docker Images (2-3 minutes)

```bash
# Create and push Docker team images
...
### Step 2b: Complete Project Setup with GitHub (1 minute)

```bash
# Navigate to project directory
cd penguin-bills

# Complete full project setup with GitHub integration
# Almost everything comes from config or Step 2a files
zzcollab -I shell

# Only flag needed:
# -I shell: Use shell/R console interface
#
# These come from config (set in Step 1):
# -G: from auto-github config (true)
#
# These are inferred from Step 2a files:
# - Team name: from DESCRIPTION or config.yaml
# - Project name: from directory name or DESCRIPTION
# - Build mode: from config.yaml
# - Dotfiles: from .zshrc_docker presence
```

**What this does:**
1. Creates complete project structure (R/, analysis/, tests/, etc.)
2. Creates GitHub repository automatically (public by default)
3. Sets up CI/CD workflows for automated testing
4. Initializes renv for dependency tracking
...
## Step 4: Create Analysis Report (2 minutes)
...
Inside the container, create your analysis as an Rmd file using your preferred text editor:

```bash
# Create the report file (use vim, nano, or your preferred editor)
...
# Save plot
ggsave("analysis/figures/bill_scatter.png", plot, width = 8, height = 6)
```
...
Still inside the Docker container (from `make docker-zsh`), add tests for computational correctness:
...
zzcollab -I shell  # Complete setup without GitHub
gh repo create rgt47/penguin-bills --private --source=. --remote=origin --push
```
...
# Clean rebuild (two-step process using config defaults)
docker system prune
rm -rf penguin-bills  # Remove failed directory
zzcollab -i -p penguin-bills  # Uses config: team-name, build-mode, dotfiles-dir
cd penguin-bills
zzcollab -I shell  # Uses config: auto-github; infers: team, project, build-mode
```
...
- **Step 2a** (Team images): 2-3 minutes
- **Step 2b** (Full setup + GitHub): 1 minute
- **Step 3** (Make private): 30 seconds
- **Step 4** (Analysis Rmd report): 2 minutes
- **Step 5** (Unit tests): 2 minutes
- **Step 6** (Validate & commit): 1 minute
- **Step 7** (Verify CI/CD): 30 seconds
- **Step 8** (Configure report rendering): 2 minutes (optional)

**Total**: ~10-11 minutes for complete 5-level reproducible analysis
(~13 minutes with optional Step 8)
